...
def safe_get_price_column(df):
    """
    Try to get the 'Close' price series.
...
def prepare_data(df, symbol, only_buy_signals):
    ...
    if price is None:
        ...
        # Ensure we have a 'Close' column for consistency
...
def run_inverse_head_and_shoulders_strategy(only_buy_signals=False, watchlist_name=None):
    """
...
def run_rectangle_pattern_strategy(only_buy_signals=False, watchlist_name=None):
    """
    ...
    for symbol in stock_list:
        try:
            ...
            # Calculate rolling max and min to find potential rectangle boundaries
            ...
            # Check for breakout above the rectangle
...
def run_ascending_channel_strategy(only_buy_signals=False, watchlist_name=None):
    """
    ...
    for symbol in stock_list:
        try:
            ...
            if peak_slope > 0 and trough_slope > 0 and abs(peak_slope - trough_slope) / abs(peak_slope) < 0.2:
                ...
                if not only_buy_signals:
                    print(f"{Colors.RED}{symbol}: No Ascending Channel pattern detected.{Colors.RESET}")
